...
parser.add_argument('--epoch', default=35)
parser.add_argument('--batch_size', default=96)
parser.add_argument('--path_train', default='./train/train')
parser.add_argument('--path_val', default='./train/val')
...
net = resnest101(pretrained=True, root='pretrained')
fc_features = net.fc.in_features
net.fc = nn.Linear(fc_features, 4)
print(net)
net = nn.DataParallel(net, device_ids=[0, 1, 2, 3]).to(device)  # 多卡训练指定GPU


# 定义损失函数和优化方式
# criterion = nn.FocalLoss()  # 损失函数为交叉熵，多用于多分类问题,此标准将LogSoftMax和NLLLoss集成到一个类中。
...
scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer=optimizer, milestones=[10, 20, 28], gamma=0.1)
        ...
        with open(args.output+"train.txt", "a")as f2:
            for epoch in range(pre_epoch, epoch):  # 从先前次数开始训练
                ...
                for i, data in enumerate(trainloader,
                    ...
                    print('[epoch:%d, iter:%d] Loss: %.03f | Acc: %.3f%% | lr: %.6f'
                          % (epoch + 1, (i + 1 + epoch * length), sum_loss / (i + 1), 100. * correct / total, optimizer.param_groups[0]['lr']) )
                    f2.write('%03d  %05d | Loss: %.03f | Acc: %.3f%% | lr: %.6f'
                             % (epoch + 1, (i + 1 + epoch * length), sum_loss / (i + 1), 100. * correct / total, optimizer.param_groups[0]['lr']))
                    f2.write('\n')
                    f2.flush()
