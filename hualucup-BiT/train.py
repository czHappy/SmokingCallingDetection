...
def mktrainval(args, logger):
  ...
  train_tx = tv.transforms.Compose([
    ...
    tv.transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5)),
      # tv.transforms.Normalize((0.485,0.456,0.406), (0.229,0.224,0.225)), imagenet
      #tv.transforms.Normalize((0.4119803003402779, 0.40889004660796374, 0.4111146789894178),
      #                        (0.2722762093287647, 0.26526443370851177, 0.27543206638572)),
  ])
  val_tx = tv.transforms.Compose([
      tv.transforms.Resize((crop, crop)),
      tv.transforms.ToTensor(),
      #tv.transforms.Normalize((0.40702813539470534, 0.4005526078656559, 0.39464261693601377),
      #                     (0.28119672440760013, 0.26599737107605065, 0.2687406301502049)),
      tv.transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5)),
  ])
  ...
  if args.dataset == "cifar10":
    ...
    train_set = tv.datasets.ImageFolder(pjoin(args.datadir, "train"), train_tx)
    valid_set = tv.datasets.ImageFolder(pjoin(args.datadir, "val"), val_tx) # 修改
  else:
  ...
  for b, (x, y) in enumerate(data_loader):
    with torch.no_grad():
      ...
      with chrono.measure("eval fprop"):
        ...
        top1, top5 = topk(logits, y, ks=(1, 3))
...
def main(args):
  ...
  model_dir = pjoin('./models', args.name)
  ...
  import label_smooth
  cri = label_smooth.LabelSmoothCELoss().to(device)
  # cri = torch.nn.CrossEntropyLoss().to(device)
  # cri = FocalLoss().to(device)
    ...
    for x, y in recycle(train_loader):
      ...
      if accum_steps == args.batch_split:
        ...
        if args.eval_every and step % args.eval_every == 0:
          all_c, all_top1, _ = run_eval(model, valid_loader, device, chrono, logger, step)
          if args.save and step >= 400: #这里修改了
            torch.save({
              "step": step,
              "model": model.state_dict(),
              "optim": optim.state_dict(),
            }, savename)
            torch.save(model.state_dict(), '%s/net_%03d.pth' % (model_dir, step))
      end = time.time()
